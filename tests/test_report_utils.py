...
from lib.report_utils import basic_bib_metadata, normalize_run_data, normalize_overall_run_data
from lib.models.search_target_response import SearchTargetResponse


def test_normalize_run_data():
    results = [
        SearchTargetResponse(elapsed=100, count=100, response={"metric_score": 0.5}, target=None),
        SearchTargetResponse(elapsed=400, count=125, response={"metric_score": 0.75}, target=None),
    ]
...
def test_normalize_overall_run_data():
    results = [
        ...
        [
            SearchTargetResponse(elapsed=200, count=100, response={"metric_score": 0.7}, target=None),
            SearchTargetResponse(elapsed=600, count=125, response={"metric_score": 0.85}, target=None),
        ],
    ]
